...
| `AGENTS.md` at project root (Codex's native convention) or `.agents/skills/` present; user mentions Codex CLI | Codex CLI          |
...
2. Else inspect project markers (`.cursor/`, `.claude/`, `AGENTS.md`/`.agents/skills/`, `.vscode/`, `.idea/`)
...
```
Host: <cursor|claude-code|codex-cli|vscode|jetbrains|generic>
...
- **Codex CLI**: yes, own format (`codex mcp add` / `~/.codex/config.toml` or project `.codex/config.toml` with `trust_level = "trusted"`) — see `framework/_shared/harness-capabilities.md`
- **VS Code**: yes (MCP settings / `.vscode/mcp.json` depending on client)
- **JetBrains**: unknown/partial — many installs have Jira _UI_ plugins but not agent MCP; prefer generic MCP if the AI assistant supports it
