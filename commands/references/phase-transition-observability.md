...
**Telemetry note**: this pipeline does not track token/cost usage itself. Earlier rounds tried a
custom per-phase Usage/Total system (`EMIT_PHASE_OBSERVABILITY`); two consecutive real smoke
tests on Codex and Claude Code showed it never actually fired in practice (work ran inline or via
native subagent, with nothing to capture), so it was removed rather than kept as unreliable
infrastructure. For usage/cost visibility, use your harness's own native tooling (Claude Code's
own usage views, Codex's own, Cursor's own) — that lives entirely outside this pipeline.