...
| _(no agents/ or commands/ folder populated)_ | See gap notes below                                                                                                                                                           |
...
- **No agent-role folder populated.** The 12 canonical `agents/*.md` files describe specialist behavior (validator, explorer, implementer, etc.) written against Claude Code's subagent frontmatter (`tools:`, `model:`, `isolation:`). Codex CLI's own subagent mechanism (`/agent`, `~/.codex/agents/*.toml`) uses a different config format this adapter does not yet generate. Until that translation exists, treat the 12 agent files as **reference material** the main Codex session reads and follows directly, not as auto-discovered subagents.
...
- **`ISOLATED_WORKSPACE` is not supported.** Codex subagents inherit the parent's sandbox policy; there is no per-subagent git-worktree equivalent. The (currently roadmap, not yet shipped) parallel-task execution mode described in `sdd-implementer.md` cannot run on this adapter — sequential execution only.

## `DELEGATE_ISOLATED` on this adapter — the concrete, end-to-end procedure (not just a capability claim)

The Validator Independence Protocol (`agents/sdd-validator-runner.md`) requires `DELEGATE_ISOLATED`: the validator must not see the implementer's reasoning, only file paths + rules. This is the highest-stakes capability gap in the pack, so it does not get a one-line "Partial" label — here is exactly what happens today, step by step, and what does not.
...
2. The **calling session** builds the scrubbed prompt itself — same scrubbing rule as the Claude Code `Task(subagent_type="sdd-validator-runner", prompt=...)` call: only the file list + the Security/Performance/Quality tables from `agents/sdd-validator-runner.md` (or, for Layer 3 mode, the extended check categories), **never** the implementation rationale.
