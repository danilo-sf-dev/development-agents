...
| B-15 | **Install scripts**                         | Shell out to or fetch any external install script — the installer agent only uses its own Read/Write/Bash tool calls                                                                                                                                                                                                                                                                                                                                                                                                   |
| B-16 | **Root agent files — non-idempotent write** | Write `AGENTS.md` or `CLAUDE.md` at project target root **except** via the idempotent, section-scoped merge described in `commands/references/project-instructions-sync.md` (create-or-append-or-replace only the marked `## SDD Kit` section). Overwriting the rest of the file, or writing outside that mechanism, is forbidden. Writing the marked section itself is expected behavior for the Claude Code and Codex CLI adapters — see `framework/_shared/harness-capabilities.md` (`WRITE_PROJECT_INSTRUCTIONS`). |
| B-17 | **Unrequested scope**                       | Commit or modify files the user did not ask for                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| B-18 | **Working directory**                       | Run SDD workflow commands with cwd inside the pack hub only, expecting it to double as a target project                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- 🚫 JetBrains _Jira Connector_ plugin ≠ MCP for the AI agent
