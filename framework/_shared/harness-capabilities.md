...
| Cursor      | No synchronous in-session delegate-and-return primitive exists, but real isolation is achieved via a **headless child process**: `agent -p --model "<resolved>" --output-format json "<scrubbed task, no rationale>"` (Cursor CLI, documented `--model`/`-p`/`--output-format` flags — see `adapters/cursor/README.md` § Model Routing). This is automated (no operator paste-back) and gives a genuinely fresh context (separate process), unlike the older "open a tab by hand" degradation. Not live-round-trip-tested in this pack's development environment — see the adapter README for exactly what was/wasn't confirmed.                                                                                                                                                                                                                                                                                                                                                                                                        | **Partial — automated via child CLI process, isolation real, live-verification pending** |
| Codex CLI   | **Automated via child process**: `codex exec --model <resolved> -c 'model_reasoning_effort="<level>"' "<scrubbed prompt, no rationale>"` — `codex exec` is Codex CLI's documented non-interactive automation mode, dispatched by the calling session itself (no operator action). That child process follows `skills/sdd-validator/SKILL.md` directly and returns the verdict on stdout — see `adapters/codex/README.md` § "Model Routing" for the full step-by-step (this replaces the earlier "operator opens a fresh session by hand" description, which undersold what `codex exec` can already do). Not live-round-trip-tested in this pack's development environment. If isolation isn't actually followed, the verdict does not satisfy the protocol — escalate to human review instead. | **Partial — automated via child CLI process, isolation real, live-verification pending** |
...
| Cursor      | Headless child process (`agent -p --model "<resolved>" --output-format json "<scrubbed task>"`, see `adapters/cursor/README.md`) — automated, real fresh context, not live-verified in this environment.                                                                                                        | **Partial — automated via child CLI process, live-verification pending**       |
| Codex CLI   | Automated child process (`codex exec --model <resolved> -c 'model_reasoning_effort="<level>"' "<scrubbed prompt>"`, see `adapters/codex/README.md`) follows the `sdd-validator` Skill's isolated-mode content directly and returns the verdict — not live-verified in this environment. | **Partial — automated via child CLI process, live-verification pending**   |
...
corrected once `agent -p`'s headless mode was verified: a child CLI process the calling session
shells out to and waits on **is** a synchronous delegate-and-return primitive, just realized as a
subprocess rather than an in-session tool call (see `DELEGATE_ISOLATED`/`VALIDATOR_ISOLATED` rows
above and `adapters/cursor/README.md`). What's still real about the Cursor gap is narrower than
before: not "no synchronous primitive exists," but "the mechanism is a documented CLI flag, not yet
live-round-trip-verified in this pack's development environment." Every adapter that hits a real gap
must say so explicitly in its own docs, not silently degrade and claim parity.
...
- **Cursor — Supported (adapter, with documented capability gaps).** Skills/commands/rules and Model Routing are Full/automated (via headless `agent -p --model ...` child dispatch, live-verification pending); structured questions (`ASK_USER`) still degrade gracefully with an explicit warning.
- **Codex CLI — Experimental.** Good 2026 coverage (skills, `AGENTS.md`, real subagents, MCP, scriptable `codex exec --model ...` for automated Model Routing and `DELEGATE_ISOLATED`/`VALIDATOR_ISOLATED`), but no structured `ASK_USER` and no `ISOLATED_WORKSPACE` (parallel task execution) — those two gaps are unrelated to validator isolation and remain real regardless of the Model Routing work. Not validated against a live Codex CLI session by this pack's maintainers; treat as experimental until someone confirms it end-to-end.
